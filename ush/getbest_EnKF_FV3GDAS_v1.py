"""
 ...
 Usage: python getbest_EnKF.py -d [COM_EnKF] -v [valid time in YYYYMMDDHH] -t [hours] -r [resolution]
                               ...
                               --minsize=x --o3fname=filename --4d=[start,stop,inchrs] --gfs_netcdf=yes/no -h
 ...
 --gfs_netcdf=yes/no : Whether to look for input EnKF files from the GFS in netcdf format (Deafult is NO).
...
def usage():
    ...
    print(" --gfs_netcdf=yes/no : Whether to look for input EnKF files from the GFS in netcdf format (Deafult is NO).")
...
def write_filelist(fname,comenkf,fsave,svdate,retro,path,suf,o3fname,getmean,gfs_netcdf):
    ...
    if retro:
        ...
        path=comenkf+'.'+svPDY+'/'+svCYC+'/atmos'
    f=open(fname,'w')
    havefile=True   
    initim=datetime.strptime(svPDY,'%Y%m%d')
    deltafcst=timedelta(hours=int(svCYC)+fsave)
    vldtim=initim+deltafcst
    vldtime_str=vldtim.strftime('%Y%m%d%H')
    if getmean:
        if gfs_netcdf:
            en=path+'/gdas.t'+svCYC+'z.atmf'+str(fsave).zfill(3)+'.ensmean.nc'
        else:
            en=path+'/sfg_'+svcdate+'_fhr'+str(fsave).zfill(2)+'_ensmean'+suf
#cltorg        f.write(en+'\n')
        f.write(en+" "+vldtime_str+'\n')
        # Also symlink file locally
        linkname='ens_mean'
    ...
    while havefile: # first while loop code
        ...
        mem=str(n).zfill(3)
        if gfs_netcdf:
            en=path+'/mem'+mem+'/gdas.t'+svCYC+'z.atmf'+str(fsave).zfill(3)+'.nc'
        else:
...
def checkmembers(thispath,fhr,thiscdate,thissuf,gfs_netcdf):
    n=0
    havefile=True
    ...
    while havefile:
        ...
        if gfs_netcdf:
            CYC=thiscdate[8:10] 
            f=thispath+'/mem'+mem+'/gdas.t'+CYC+'z.atmf'+str(fhr).zfill(3)+'.nc'
        else:
...
def main():
    ...
    try:
        opts, args = getopt.getopt(sys.argv[1:], "d:v:t:r:o:m:s:h",["retro=","exact=","minsize=","o3fname=","4d=","gfs_netcdf="])
    ...
    if gfs_netcdf and suf !="":        
        suf=""
        print("When gfs_netcf=YES we must use hi-res EnKF members! Resetting resolution ot hi-res!")
                ...
                if found[x] is False:                
                   ...
                   # if False in found:
                        if retro:
                            ...
                            path=comenkf+'.'+PDY+'/'+CYC+'/atmos'                
                        if gfs_netcdf:
                            en=path+'/gdas.t'+CYC+'z.atmf'+str(f).zfill(3)+'.ensmean.nc'
                        else:
...
#cltorg        for fhr in found:
        ...
        if svdate is None: 
            print('Unable to find matching EnKF members.')
            sys.exit()
        write_filelist(fname,comenkf,fsave,svdate,retro,path,suf,o3fname,getmean,gfs_netcdf)
