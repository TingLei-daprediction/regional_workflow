...
if [ $ictype = oldgfs ]; then   # input data is old spectral sigio format.
 ...
 if [ ${ATMANL:-"NULL"} = "NULL" ]; then
  if [ -s ${INIDIR}/siganl.${CDUMP}.$CDATE ]; then
   export ATMANL=$${ATMANL:-$INIDIR/siganl.${CDUMP}.$CDATE}
   export SFCANL=${SFCANL:-$INIDIR/sfcanl.${CDUMP}.$CDATE}
  else
   export ATMANL=${ATMANL:-$INIDIR/${CDUMP}.t${cyc}z.sanl}
   export SFCANL=${ATMANL:-$INIDIR/${CDUMP}.t${cyc}z.sfcanl}
  fi
 fi
...
#------------------------------------------------
  ...
  tmpvar0=`ls  ${DATA}/gfs_data.tile*.nc` 
  tmpvar="${tmpvar0##*/}" 
#clt  mv ${DATA}/gfs_ctrl.nc        $OUTDIR/{chg_memstring}gfs_ctrl.nc
#clt  mv ${DATA}/gfs_ctrl.nc        $OUTDIR/{chg_memstring}gfs_ctrl.nc
  mv ${DATA}/$tmpvar  $OUTDIR/${chg_memstring}$tmpvar
  mv ${DATA}/gfs_ctrl.nc        $OUTDIR/${chg_memstring}gfs_ctrl.nc
  if [ $gtype = regional ]; then
#cltorg    mv ${DATA}/gfs_bndy.tile7.nc $OUTDIR/${chg_memstring}gfs_bndy.tile7.000.nc
    mv ${DATA}/gfs_bndy.tile7.nc $OUTDIR/${chg_memstring}gfs_bndy.tile7.000.nc
  fi
...
#---------------------------------------------------
  ...
  if [ $gtype = regional ]; then
    $CHGRESSH_local
    mv ${DATA}/out.sfc.tile${TILE_NUM}.nc $OUTDIR/${chg_memstring}sfc_data.tile${TILE_NUM}.nc
    rc=$?
    if [[ $rc -ne 0 ]] ; then
      echo "***ERROR*** rc= $rc"
      exit $rc
    ...
    while [ $tile -le $ntiles ]; do
      ...
      mv ${DATA}/out.sfc.tile${tile}.nc $OUTDIR/${chg_memstring}sfc_data.tile${tile}.nc
      tile=`expr $tile + 1 `
    done
  fi
  ...
  export ATMANL=${ATMANL:-$INIDIR/${CDUMP}.t${cyc}z.atmf${bchour}.nemsio}
  export SIGINP=$ATMANL
  export SFCINP=NULL
  export NSTINP=NULL
  export LATB=$LATB_ATM
...
#
  echo "herethinkdeb is "$CHGRESSH_local
  ...
  mv ${DATA}/gfs_bndy.tile7.nc $OUTDIR/${chg_memstring}gfs_bndy.tile7.${bchour}.nc
